...
class Solution {
    ...
    public String solution(int n, int k, String[] cmd) {
        ...
        for(String commands : cmd){
            ...
            if(command == 'U') U(Integer.valueOf(commands.substring(2)));
            if(command == 'D') D(Integer.valueOf(commands.substring(2)));
            if(command == 'C') C();
            if(command == 'Z') Z();
        }
        
        for(int i = 0; i < n; ++i){
            char result = (tS.contains(i) ? 'O' : 'X');
...
/*
    ...
    이중 LinkedList로 푸는 게 모범 답안이어서 효율성 5개 테스트 케이스가 추가됨.
    삽입 삭제를 빈번하게 하는 테스트 케이스라 Set으로 풀면 효율성 5개 틀림.
*/