...
/*
문제 3-4) 인픽스를 프리픽스로 변환하기
	인픽스로 주어진 식을 프리픽스로 변환하는 코드를 작성하라. 수식은 사칙연산 (+, -, *, /)만 사용한다고 가정한다.
...
풀이 ) 1.infix를 거꾸로 읽는다.
	   2.숫자면 string += 한다.
	   3.연산자인데 우선순위가 높이면 S.push()한다.
	   4.연산장인데 우선순위가 낮으면 우선순위가 낮거나 같은 연산자가 나올때까지 string += S.top() && S.pop()한다.
	   5.읽기가 종료되면 S.empty()일때까지 string += S.top() && S.pop()한다.
	   6. string을 역순으로 뒤집는다.
...
string infixToPrefix(string input) {
	...
	for (int i = reversePrefix.size() - 1; i >= 0; i--) {
		prefix += reversePrefix[i];
	}
	return prefix;
}
...
int main() {
	string example = "(1 - (2 + 3)) * 5";	// => *-1+235

	cout << infixToPrefix(example) << endl;
