...
int solution(vector<string> user_id, vector<string> banned_id) {
	...
	for (int i = 0; i < m; i++) {
		...
		for (int j = 0; j < banned_id[i].size(); j++) {
			if (banned_id[i][j] == '*') starV.push_back(j);
		}
		for (int j = 0; j < n; j++) {
			string temp = user_id[j];
			for (int k = 0; k < starV.size(); k++) {
				if (starV[k] >= temp.size()) break;
				temp[starV[k]] = '*';
			}
			if (temp == banned_id[i]) idxVArr[i].push_back(j);
		}
	}
	//log
	// for(int i = 0; i < m; i++){
	//     for(int j = 0; j < idxVArr[i].size(); j++){
	//         cout << idxVArr[i][j] << " ";
	//     }
	//     cout << "\n";
	// }
...
void DFS(int L) {
	if (L == m) {
		...
		for (int i = 0; i < idxVArr[L].size(); i++) {
			if (flag[idxVArr[L][i]] == 0) {
				flag[idxVArr[L][i]] = 1;
				DFS(L + 1);
				flag[idxVArr[L][i]] = 0;
			}
