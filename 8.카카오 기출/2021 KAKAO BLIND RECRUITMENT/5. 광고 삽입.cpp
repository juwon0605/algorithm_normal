...
string solution(string play_time, string adv_time, vector<string> logs) {
	string answer;
	string temp;
	int n, window, start, end, res = 0;
	long long max = 0, accum = 0;
	setHMS(play_time);
	n = 3600 * h + 60 * m + s;
	vector<int> timeV(n);
	setHMS(adv_time);
	window = 3600 * h + 60 * m + s;
	for (int i = 0; i < logs.size(); i++) {
		temp = logs[i].substr(0, 8);
		setHMS(temp);
		start = 3600 * h + 60 * m + s;
		temp = logs[i].substr(9, 8);
		setHMS(temp);
		end = 3600 * h + 60 * m + s;
	...
	//log
	// cout << printTime(max) << "\n";
	answer = printTime(res);
	return answer;
}

void setHMS(string str) {
	h = stoi(str.substr(0, 2));
	m = stoi(str.substr(3, 2));
	s = stoi(str.substr(6, 2));
}

string printTime(int second) {
	string temp;
	h = second / 3600;
	m = (second % 3600) / 60;
	s = second % 60;
