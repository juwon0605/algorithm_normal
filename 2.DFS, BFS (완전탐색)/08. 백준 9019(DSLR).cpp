...
void BFS(int a, int b) {
	...
	while (!Q.empty()) {
		...
		Q.pop();
		int num = now.first;
		string path = now.second;
		for (int i = 0; i < 4; i++) {
			int next = calDSLR(i, num);
			...
			flag[next] = 1;
			Q.push(make_pair(next, path + DSLR[i]));
		}
	}
}

int calDSLR(int idx, int num) {
	int res;
	if (idx == 0) {
		res = num * 2;
...
/*
모범 답안
	88번 줄 res == 0(X)(res == -1)(O)으로 문제를 잘못 이해해서 한참 찾음...
	핵심 개념을 찾고 구현했는데 오답이 뜨면
	문제를 꼼꼼히 다시 검토해보자!
*/