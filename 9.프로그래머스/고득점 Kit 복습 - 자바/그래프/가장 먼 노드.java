...
class Solution {
    public int solution(int n, int[][] edges) {
        ...
        edgeQ.add(new Edge(0,0));
        
        while(!edgeQ.isEmpty()){
            int now = edgeQ.peek().v2;
            int accum = edgeQ.peek().accum;
            edgeQ.remove();
            ...
            for(int i = 0; i < graph[now].size(); ++i){
                ...
                if(distances[next] > accum + 1){
                    ...
                    edgeQ.add(new Edge(next, accum + 1));
                }
            }
        }
...
class Edge implements Comparable<Edge>{
    ...
    public int compareTo(Edge other){
        return Integer.compare(accum, other.accum);
    }
}
