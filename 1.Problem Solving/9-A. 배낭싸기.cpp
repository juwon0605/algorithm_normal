...
쪼개어 일부분만 배낭에 넣는 것이 가능하다. 배낭에 담은 물건들의 가치의
...
3. 세 번째 줄에는 각 물건의 무게 W(1 ≤ W ≤ 100,000)가 빈 칸 하나를 사
이에 두고 순서대로 주어진다.
4. 네 번째 줄에는 각 물건의 가치 V(1 ≤ V ≤ 100,000)가 빈 칸 하나를 사
이에 두고 순서대로 주어진다. (단, 모든 물건에 대해 V/W는 자연수이다.)
...
int main() {
	...
	while (t--) {
		...
		sort(itemV.begin(), itemV.begin() + n);
		while (accumWeight + itemV[idx].weight < k) {
			accumWeight += itemV[idx].weight;
			res += itemV[idx].value;
			idx++;
		}
		/*
		...
		res -= itemV[idx].value;
		*/
		res += itemV[idx].value / itemV[idx].weight * (k - accumWeight);
		cout << res << endl;
	}
	return 0;
}
